package com.hkc.mediarecodertest
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onDestroy() {
        ...
        recorder.release()            //釋放錄音器占用資源
        player.release()                //釋放播放器占用資源
        super.onDestroy()
    ...
    //將變數與XML元件綁定
        ...
        btn_stop_record.setOnClickListener {                        //設定停止錄音的監聽器
                try{              //若使用模擬器停止錄音容易產生例外，所以使用try-catch處理
                    val file = File(folder, fileName)       //定義錄音檔案
                    recorder.stop()                     //結束錄音
                    textView.text = "已儲存至${file.absolutePath}"
                    btn_record.isEnabled = true         //開啟錄音按鈕
                    btn_stop_record.isEnabled = false      // 關閉停止錄音按鈕按鈕
                    btn_play.isEnabled = false              // 關閉播放按鈕按鈕
                    btn_stop_play.isEnabled = false         // 關閉停止播放按鈕按鈕
        ...
        btn_play.setOnClickListener {                   //設定開始播放的監聽器
            ...
            btn_stop_record.isEnabled = false      // 關閉停止錄音按鈕按鈕
            btn_play.isEnabled = false              // 關閉播放按鈕按鈕
            btn_stop_play.isEnabled = true         // 開啟停止播放按鈕按鈕
        ...
        btn_stop_play.setOnClickListener{
            ...
            btn_record.isEnabled = true         //開啟錄音按鈕
            btn_stop_record.isEnabled = false      // 關閉停止錄音按鈕按鈕
            btn_play.isEnabled = true              // 開啟播放按鈕按鈕
            btn_stop_play.isEnabled = false         // 關閉停止播放按鈕按鈕
